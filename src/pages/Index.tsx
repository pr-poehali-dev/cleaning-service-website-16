...
const services = [
  ...
  { icon: "Sofa", title: "Химчистка мебели", desc: "Глубокая чистка диванов, кресел, ковров и матрасов", price: "от 1 200 ₽" },
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur shadow-md" : "bg-transparent"}`}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#00c9a7] to-[#0097e6] flex items-center justify-center">
              <Icon name="Sparkles" size={18} className="text-white" />
            </div>
            <span className="font-black text-xl tracking-tight text-[#141d26]">Чисто<span className="text-[#00c9a7]">Дом</span></span>
          </div>
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-semibold text-[#4a5568]">
            {[["about","О нас"],["services","Услуги"],["portfolio","Портфолио"],["reviews","Отзывы"],["contacts","Контакты"]].map(([id,label]) => (
              <button key={id} onClick={() => scrollTo(id)} className="hover:text-[#00c9a7] transition-colors">{label}</button>
            ))}
          </nav>
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden p-2">
            <Icon name={menuOpen ? "X" : "Menu"} size={24} className="text-[#141d26]" />
          </button>
        </div>

        {menuOpen && (
          <div className="md:hidden bg-white border-t px-6 py-4 flex flex-col gap-4 text-sm font-semibold text-[#4a5568]">
            {[["about","О нас"],["services","Услуги"],["portfolio","Портфолио"],["reviews","Отзывы"],["contacts","Контакты"]].map(([id,label]) => (
              <button key={id} onClick={() => scrollTo(id)} className="text-left hover:text-[#00c9a7]">{label}</button>
            ))}
            <button onClick={() => { setModalOpen(true); setMenuOpen(false); }} className="mt-2 bg-gradient-to-r from-[#00c9a7] to-[#0097e6] text-white font-bold px-5 py-2.5 rounded-full">
              Заказать уборку
            </button>
          </div>
        )}
